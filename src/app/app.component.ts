  ...
  ngOnInit() {
    this._emp.getSales_MTD().subscribe((data) => {
      ...
      this.barChartLabels.forEach((label) => {
        var yourRank = data.find((x) => x.label == label)['yourRank'];
        var total = data.find((x) => x.label == label)['total'];
        this.barChartData_MTD[0].data.push(yourRank);
        this.barChartData_MTD[1].data.push(total - yourRank);
      });
    });
    ...
    this._emp.getSales_lastMonth().subscribe((data) => {
      ...
      this.barChartLabels.forEach((label) => {
        var yourRank = data.find((x) => x.label == label)['yourRank'];
        var total = data.find((x) => x.label == label)['total'];
        this.barChartData_lastMonth[0].data.push(yourRank);
        this.barChartData_lastMonth[1].data.push(total - yourRank);
      });
    });
    ...
    //     var yourRank = data.find((x) => x.label == label)['yourRank'];
    //     var total = data.find((x) => x.label == label)['total'];
    //     this.barChartData_YTD[0].data.push(yourRank);
    //     this.barChartData_YTD[1].data.push(total - yourRank);
    //   });
    //});
  }
}
